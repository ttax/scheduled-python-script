...
def read_fund_codes_from_file(file_path=None):
...
def get_fund_net_worth(fund_code):
    ...
    for attempt in range(3):
        try:
            ...
            def format_value(value):
                try:
                    if '-' in str(value):
                        return f'<font color="#00FF00">{value}</font>'
                    else:
                        return f'<font color="#FF0000">{value}</font>'
                except:
                    return value
...
"""
            ...
            print(f"尝试 {attempt + 1}/3 获取基金 {fund_code} 数据失败: {e}")
            time.sleep(2)
